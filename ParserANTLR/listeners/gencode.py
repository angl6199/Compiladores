...
class GenCode(marzoListener):
    ...
    def exitLetra(self, ctx: marzoParser.PrimariaContext):
        self.var += 1
        self.ifCounter += 1
        self.queue.append(self.var)
        self.stack.append(self.var)
    ...
    def exitSuma(self, ctx: marzoParser.SumaContext):
        if(self.ifCounter > 3 and self.isIf):
            print("ELSE:")
        print("ADD $v" + str((self.var)) + ", " + "$v" +
              str(self.stack.pop()) + ", " + "$v" + str(self.stack.pop()))

    def exitAsignacion(self, ctx: marzoParser.AsignacionContext):
        if(self.ifCounter > 3):
            print("ELSE:")
        print("li $v" + str((self.var)) + ", " + "$v" +
              str(self.stack.pop()) + ", " + "$v" + str(self.stack.pop()))
    ...
    def exitMayor(self, ctx: marzoParser.MayorContext):
        print("stg $v" + str((self.var)) + ", " + "$v" +
              str(self.stack.pop(1)) + ", " + "$v" + str(self.stack.pop()))

    def exitMenor(self, ctx: marzoParser.MenorContext):
        print("slt $v" + str((self.var)) + ", " + "$v" +
              str(self.stack.pop(1)) + ", " + "$v" + str(self.stack.pop()))
    ...
    def exitIf(self, ctx: marzoParser.IfContext):
        print("beq $v" + str(self.queue.pop(0)) +
              ", $v" + str(self.queue.pop(0)) + ", ELSE")
        self.isIf = False
        self.ifCounter = 0
